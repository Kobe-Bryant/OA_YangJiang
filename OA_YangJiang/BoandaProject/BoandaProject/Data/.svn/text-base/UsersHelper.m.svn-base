...
- (NSString *)queryDeptIDByName:(NSString *)deptName
{
    ...
    NSString *sql = [NSString stringWithFormat:@"select ZZBH from T_ADMIN_RMS_ZZJG where ZZQC=%@;", deptName];
    NSString __block *ret = nil;
    [self.dbQueue inDatabase:^(FMDatabase *db) {
        FMResultSet *rs = [db executeQuery:sql];
        ret = [rs stringForColumn:@"ZZBH"];
        [rs close];
    }];
    return ret;
}
...
- (NSString *)queryUserNameByID:(NSString *)aId
{
    ...
    NSString *sql = [NSString stringWithFormat:@"select YHMC from T_ADMIN_RMS_YH where YHID='%@';", aId];
    NSString __block *ret = nil;
    [self.dbQueue inDatabase:^(FMDatabase *db) {
        FMResultSet *rs = [db executeQuery:sql];
        if([rs next])
