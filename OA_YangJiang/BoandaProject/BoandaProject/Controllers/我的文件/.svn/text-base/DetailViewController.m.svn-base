...
- (void)deleteButtonClick:(id)sender
{
    ...
    for (int i = 0; i < self.boxArray.count; i++)
    {
        ...
        if([box isChecked])
        {
            ...
            //删除附件表中的数据
            BOOL ret = [[AttachManageService shared] deleteOneFileByPath:path];
            if(ret)
            {
                DLog(@"%@", path);
                //从数据库中删除记录
...
- (void)didSelectedRow:(NSInteger)row
{
    ...
    for (int i = 0; i < self.boxArray.count; i++)
    {
        ...
        if([box isChecked])
        {
            ...
            if([self.fileManager fileExistsAtPath:fromPath])
            {
                ...
                //跟新附件表中的路径信息
                //删除附件表中的数据
                BOOL ret = [[AttachManageService shared] updateOneFileByOldPath:fromPath toNewPath:toPath];
                if(ret)
                {
                    DLog(@"The old path is:%@\nThe new path is:%@", fromPath,toPath);
                    //从数据库中删除记录
                }
            }
