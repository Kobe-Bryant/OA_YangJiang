...
@interface ReplyMeetingViewController ()
...
@implementation ReplyMeetingViewController

@synthesize  txtView,webHelper,tzbh,requestDataType,TID;
...
- (void)viewDidLoad
{
    ...
    [params setObject:@"QUERY_HYFK" forKey:@"service"];
...
-(IBAction)btnPressed:(id)sender
{
    ...
    if(self.TID == nil || self.TID.length == 0)
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"暂时不支持提交反馈内容." delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil,nil];
        [alert show];
        return;
    }
