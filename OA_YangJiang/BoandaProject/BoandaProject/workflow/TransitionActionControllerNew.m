...
-(void)processWebData:(NSData*)webData{
    ...
    if (webServiceType == kWebService_Transfer) {
        ...
        if (dicTmp && [[dicTmp objectForKey:@"result"] isEqualToString:@"success"]) {
            ...
            NSArray *tmpParserdAry = [dicTmp objectForKey:@"steps"];
            NSMutableArray *stepsAry = [[NSMutableArray alloc] init];
            for(int i = 0; i < tmpParserdAry.count; i++)
            {
                NSDictionary *subDict = [tmpParserdAry objectAtIndex:i];
                if(![self.filterStepsAry containsObject:[subDict objectForKey:@"stepDesc"]])
                {
                    [stepsAry addObject:subDict];
                }
            }
...
-(void)updateSelectStep{
    ...
    selPersonItem.arySelectedReaderUsers = nil;
    [stepTableView reloadData];
    [usrTableView reloadData];
}
